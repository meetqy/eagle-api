...
module.exports = (db, name, opts) => {
  ...
  function list(req, res, next) {
    ...
    Object.keys(req.query).forEach(query => {
      ...
      for (const i in arr) {
        if (_.has(arr[i], query) || query === 'callback' || query === '_' || /_lte$/.test(query) || /_gte$/.test(query) || /_ne$/.test(query) || /_like$/.test(query) || /_null$/.test(query)) return;
      }
    ...
    Object.keys(req.query).forEach(key => {
      ...
      if (key !== 'callback' && key !== '_') {
        ...
        const isNull = /_null$/.test(key);
        const path = key.replace(/(_lte|_gte|_ne|_like|_null)$/, '');
            ...
            if (isRange) {
              ...
              if(typeof elementValue === 'object') {
                if(elementValue instanceof Array) {
                  ...
                  return Object.keys(elementValue).length
                }
              }
            } else {
              return value === elementValue.toString();
